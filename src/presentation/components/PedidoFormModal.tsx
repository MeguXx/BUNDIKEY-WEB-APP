...
export const PedidoFormModal: React.FC<Props> = ({ pedido, products, onClose, onDelete }) => {
  ...
  // Un pedido "de historial" (ENTREGADO o CANCELADO) ya se contabilizó en caja.
  // Esta comprobación es una segunda barrera (además del filtro en QuickStatusModal):
  // si por una actualización en tiempo real el pedido pasó a historial mientras
  // el formulario seguía abierto, igual queda bloqueado aquí antes de guardar.
  const esHistorial = Boolean(pedido && (pedido.estado === 'ENTREGADO' || pedido.estado === 'CANCELADO'));
  // Eliminar un pedido activo lo puede hacer CAJA o ADMIN (corregir un error de toma de pedido).
  // Eliminar un registro de historial (una venta ya cerrada) solo lo puede hacer ADMIN, y siempre con confirmación explícita.
  const puedeEliminar = Boolean(pedido) && (dbUser?.rol === 'ADMIN' || (dbUser?.rol === 'CAJA' && !esHistorial));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (esHistorial) {
      alert('Este pedido ya pertenece al Historial (venta cerrada) y no puede editarse.');
      return;
    }
    ...
    if (pedido) {
      const resumen = `${clienteNombre || (tipoAtencion === 'MESA' ? `Mesa ${numeroMesa}` : 'Cliente')} · Total S/ ${(Object.keys(selectedMap).reduce((s, k) => s + selectedMap[k].qty * selectedMap[k].price, 0) + (isEnvio ? (districtFees[distrito] || 5) : 0)).toFixed(2)}`;
      if (!window.confirm(`¿Confirmas guardar los cambios de este pedido?\n\n${resumen}`)) return;
    }
    ...
    <div style={webStyles.modalOverlay}>
      <div style={{ ...webStyles.modalContent, maxWidth: '1100px', width: '95%', padding: '20px 30px' }}>
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          {esHistorial && (
            <div style={{ padding: '12px 15px', borderRadius: '10px', backgroundColor: '#F5F5F5', border: '1.5px dashed #C9C9C9', color: '#8A8A8A', fontSize: '13px', fontWeight: 700 }}>
              PEDIDO DE HISTORIAL ({pedido?.estado.replace('_', ' ')}) — modo solo lectura, no se puede guardar ni {dbUser?.rol === 'ADMIN' ? '' : 'eliminar'}.
            </div>
          )}

          <div style={{ display: 'flex', gap: '10px' }}>
            <button type="submit" disabled={esHistorial} style={{...webStyles.primaryBtn, flex: 2, padding: '12px', opacity: esHistorial ? 0.5 : 1, cursor: esHistorial ? 'not-allowed' : 'pointer'}}>Guardar</button>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const mensaje = esHistorial
                    ? `Este es un registro del HISTORIAL (venta ya cerrada de "${nombreRef}"). Eliminarlo borra la venta de forma permanente y afectará el cierre de caja. ¿Confirmas que deseas eliminarlo?`
                    : `¿Eliminar el pedido de "${nombreRef}"? Esta acción no se puede deshacer.`;
                  if (window.confirm(mensaje)) onDelete(pedido.id);
                }}
                style={{...webStyles.deleteBtnModal, flex: 1, padding: '12px'}}
              >
                Eliminar
              </button>
            )}
